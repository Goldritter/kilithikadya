...
(defn get-hit-probabilities [& {:keys [attacks skill
  ...
  (let [separate-critical-hits? (or lethal? (not= 0 sustained))
        ...
        to-hit-critically-probability (if separate-critical-hits? 0 (get-to-hit-probability 6 0 reroll-attack? true))
        ...
        hits-probability-map (if separate-critical-hits?
                               (apply merge-with + (map #(sorted-map (+ (first %) (* (if lethal? sustained (inc sustained)) (second %)))
                               ...
                               (apply merge-with + (map #(sorted-map % (.probability to-hit-distribution %))
                                                        attack-range)))
        ]
...
(defn get-probabilities-for [& {:keys [attacks skill strength ap damage anti
  ...
  (let [separate-critical-wounds? devastating?
        ...
        wound-probabilities (apply merge-with +
                                   (doall (pmap #(get-occurence-probability-map
